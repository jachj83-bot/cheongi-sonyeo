  ...
  return (
    <>
      ...
      <div style={{position:'sticky',top:0,zIndex:100,background:'rgba(11,10,31,0.95)',backdropFilter:'blur(12px)',borderBottom:'1px solid rgba(232,200,126,0.1)',padding:'16px 24px',display:'flex',justifyContent:'space-between',alignItems:'center'}}>
        ...
        <div style={{fontSize:'12px',color:'rgba(232,200,126,0.5)',letterSpacing:'1px'}}>📜 토정비결</div>
      </div>

      {step === 'input' && (
        <div style={{minHeight:'100vh',background:'#0B0A1F'}}>
          <div style={{maxWidth:'480px',margin:'0 auto',padding:'40px 20px 80px'}}>
            <div style={{fontSize:'10px',letterSpacing:'4px',color:'rgba(232,200,126,0.4)',marginBottom:'12px'}}>전통 방식으로 보는 신수</div>
            <h2 style={{fontFamily:"'Cormorant Garamond', 'Noto Serif KR', serif",fontSize:'30px',fontWeight:'600',marginBottom:'8px',color:'#E8C87E',letterSpacing:'-0.5px'}}>올 한 해<br/>나의 신수를 봅니다</h2>
            <p style={{fontSize:'12px',color:'rgba(237,233,242,0.3)',marginBottom:'36px',letterSpacing:'1px'}}>사주를 바탕으로 괘를 뽑아 계절별 신수를 풀어드려요</p>
        ...
        <div style={{minHeight:'100vh',background:'#0B0A1F'}}>
          <div style={{maxWidth:'600px',margin:'0 auto',padding:'32px 20px 80px'}}>
            <div style={{background:'rgba(232,200,126,0.05)',border:'1px solid rgba(232,200,126,0.15)',borderRadius:'4px',padding:'20px',marginBottom:'20px'}}>
              <h2 style={{fontFamily:"'Cormorant Garamond', serif",fontSize:'20px',color:'#E8C87E',marginBottom:'4px'}}>{form.name}님의 토정비결</h2>
              <p style={{color:'rgba(237,233,242,0.4)',fontSize:'13px'}}>
                {gwae ? `${gwae.sanggwae}-${gwae.junggwae}-${gwae.hagwae}괘 · ` : ''}{form.year}년 {form.month}월 {form.day}일생
              </p>
            </div>
            ...
            {loading ? (
              ...
              <>
                ...
                <div style={{display:'flex',gap:'10px'}}>
                  <button onClick={()=>{setStep('input');setResult('');setGwae(null);}} style={{flex:1,background:'rgba(232,200,126,0.1)',color:'#E8C87E',padding:'14px',border:'1px solid rgba(232,200,126,0.3)',borderRadius:'4px',fontSize:'14px',cursor:'pointer'}}>
                    다시 보기
                  </button>
