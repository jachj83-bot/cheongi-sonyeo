...
export default function Home() {
  ...
  const [form, setForm] = useState({ name: '', year: '', month: '', day: '', gender: '' });
      ...
      <div style={{minHeight:'100vh',background:'#080b14',color:'#e8e0d0',fontFamily:'sans-serif'}}>
        ...
        {step === 'input' && (
          <div style={{maxWidth:'480px',margin:'0 auto',padding:'60px 24px'}}>
            ...
            <h2 style={{fontSize:'26px',fontWeight:'bold',marginBottom:'32px'}}>생년월일을 알려주세요</h2>
            <div style={{marginBottom:'20px'}}>
              <label style={{display:'block',fontSize:'13px',color:'rgba(232,224,208,0.6)',marginBottom:'8px'}}>이름</label>
              <input type="text" placeholder="홍길동" value={form.name} onChange={e=>setForm({...form,name:e.target.value})} style={{width:'100%',background:'rgba(255,255,255,0.06)',border:'1px solid rgba(255,255,255,0.12)',borderRadius:'10px',padding:'12px 16px',color:'#e8e0d0',fontSize:'15px',outline:'none'}}/>
            </div>
            <div style={{display:'grid',gridTemplateColumns:'2fr 1fr 1fr',gap:'12px',marginBottom:'20px'}}>
              {[['year','년','1995'],['month','월','03'],['day','일','15']].map(([k,l,p])=>(
                <div key={k}>
                  ...
                  <input type="number" placeholder={p} value={form[k]} onChange={e=>setForm({...form,[k]:e.target.value})} style={{width:'100%',background:'rgba(255,255,255,0.06)',border:'1px solid rgba(255,255,255,0.12)',borderRadius:'10px',padding:'12px 10px',color:'#e8e0d0',fontSize:'15px',outline:'none'}}/>
                </div>
              ))}
            </div>
